...
def check_brackets(filepath):
    ...
    while i < length:
        ...
        if char in ['(', '{', '[']:
            ...
            if not stack:
                print(f"Extra closing bracket '{char}' at character index {i}")
            ...
            if (char == ')' and open_char != '(') or \
               ...
               (char == ']' and open_char != '['):
                print(f"Mismatched brackets: opened '{open_char}' at {open_idx}, closed with '{char}' at {i}")
                line_no = content[:i].count('\n') + 1
                print(f"Around line {line_no}: ...{content[max(0, i-40):i]}>>> {char} <<< {content[i+1:min(length, i+40)]}...")
                return False
        i += 1
    ...
    if in_string:
        print(f"Unclosed string of type '{string_char}'")
        return False
        
    print(f"No basic syntax/bracket matching errors found in {filepath}!")
    return True

check_brackets("js/data.js")
