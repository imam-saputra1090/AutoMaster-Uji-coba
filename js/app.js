...
const App = {
  ...
  init() {
    try {
      ...
      if (typeof AuthManager !== 'undefined') {
        ...
        if (!savedApiUrl) {
          ...
          const isLocal = hostname === 'localhost' || hostname === '127.0.0.1' || 
                          ...
                          hostname.startsWith('172.') || hostname === ''; // file protocol
          
          if (isLocal && window.location.protocol.startsWith('http')) {
            // Local LAN Server API
            savedApiUrl = window.location.origin + '/api';
          } else {
      ...
      if (typeof VrWorkshop !== 'undefined' && typeof VrWorkshop.init === 'function') {
        VrWorkshop.init();
      }

      if (typeof BooksReader !== 'undefined' && typeof BooksReader.init === 'function') {
        BooksReader.init();
      }

      if (typeof K3Interactive !== 'undefined' && typeof K3Interactive.init === 'function') {
        K3Interactive.init();
      }
      ...
      // Selalu mulai di halaman landing (beranda awal) sesuai pedoman sayembara
  ...
  async checkEnvironment() {
    ...
    const fileLauncher = document.getElementById('file-protocol-launcher');
    const warningBanner = document.getElementById('offline-cors-warning-banner');
    ...
    if (isCorsBlocked) {
      ...
      if (fileLauncher) fileLauncher.style.display = 'block';
      if (warningBanner) warningBanner.style.display = 'block';

      const downloadBatBtn = document.getElementById('btn-download-bat');
      if (downloadBatBtn && !downloadBatBtn.dataset.bound) {
        downloadBatBtn.dataset.bound = 'true';
        downloadBatBtn.addEventListener('click', () => {
          this.triggerBatDownload();
          const inst = document.getElementById('bat-instructions');
          if (inst) inst.style.display = 'block';
        });
      }
    } else {
      if (playBtn) playBtn.style.display = 'block';
      if (fileLauncher) fileLauncher.style.display = 'none';
  ...
  triggerBatDownload() {
    try {
      ...
      folderPath = decodeURIComponent(folderPath).replace(/\//g, '\\');

      console.log('[App] Jalur folder game terdeteksi:', folderPath);
...
echo ==========================================================
echo.
echo Mencoba mengaktifkan server lokal di:
echo "${folderPath}"
echo.
...
if not %CHROME_PATH%=="" (
    ...
    %CHROME_PATH% --allow-file-access-from-files "file:///${folderPath.replace(/\\/g, '/')}/index.html"
    exit
)
...
if not %EDGE_PATH%=="" (
    ...
    %EDGE_PATH% --allow-file-access-from-files "file:///${folderPath.replace(/\\/g, '/')}/index.html"
    exit
)
...
`;
      ...
      a.href = url;
      a.download = 'JALANKAN_GAME.bat';
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
      
      this.showToast('📥 File JALANKAN_GAME.bat berhasil diunduh. Silakan jalankan file tersebut untuk memulai!', 'success', 5000);
    } catch (e) {
      console.error('[App] Gagal mengunduh file bat:', e);
      this.showToast('❌ Gagal mengunduh file bat secara otomatis. Silakan jalankan manual JALANKAN_GAME.bat di folder game Anda.', 'error');
    }
  },
  ...
  bindEvents() {
    ...
    this._bindClick('btn-logout-game', () => {
      if (typeof AuthManager !== 'undefined') {
        AuthManager.logout();
      } else {
        this.showScreen('landing');
      }
      this.showToast('👋 Anda telah keluar dari game.', 'success');
    });
    ...
    this._bindClick('btn-landing-teacher', () => {
      const pwd = prompt("Masukkan Password Guru:");
      if (pwd === 'user123' || pwd === 'AutoMaster2024!Guru') {
    ...
    this._bindClick('btn-select-mode-map', () => {
      const modal = document.getElementById('modal-mode-select');
    ...
    this._bindClick('btn-select-mode-vr', () => {
      const modal = document.getElementById('modal-mode-select');
      if (modal) modal.style.display = 'none';
      if (typeof VrWorkshop !== 'undefined') {
        VrWorkshop.activate();
      }
      this.showScreen('vr');
    });

    this._bindClick('btn-close-mode-select', () => {
      const modal = document.getElementById('modal-mode-select');
      if (modal) modal.style.display = 'none';
    });
    ...
    this._bindClick('btn-goto-tools-direct', () => {
      if (typeof BooksReader !== 'undefined') {
        BooksReader.open();
      } else {
        this.showModal('modal-tools-guide');
      }
    });

    this._bindClick('btn-close-tools-guide', () => {
      if (typeof BooksReader !== 'undefined') {
        BooksReader.close();
      } else {
        this.hideModal('modal-tools-guide');
      }
    });

    this._bindClick('btn-goto-k3-direct', () => {
      if (typeof K3Interactive !== 'undefined') {
        K3Interactive.open();
      } else {
        this.showToast('🛠️ Membuka Panduan K3 & Alat TKR...', 'info');
        this.currentLevel = 1;
    ...
    this._bindClick('btn-goto-measuring-direct', () => {
      if (typeof MeasuringTools !== 'undefined') {
        if (typeof AudioManager !== 'undefined') AudioManager.playSFX('click');
        this.showScreen('measuring');
        MeasuringTools.open();
      } else {
        this.showToast('⚠️ Modul Simulasi Alat Ukur belum dimuat.', 'warning');
      }
    });

    this._bindClick('btn-goto-rankings', () => {
      if (typeof AudioManager !== 'undefined') AudioManager.playSFX('click');
      this.showScreen('rankings');
      this.loadStudentRankings();
    });

    this._bindClick('btn-back-from-rankings', () => {
      this.showScreen('menu');
    });
   ...
   */
  showScreen(screenId) {
    // Nonaktifkan VR workshop jika meninggalkan layar VR
    if (screenId !== 'vr' && typeof VrWorkshop !== 'undefined' && VrWorkshop.isActive) {
      VrWorkshop.deactivate();
    }
    ...
    if (screenId === 'login') {
      ...
      // Clear actual input fields just in case browser cache/autocomplete overrides reset()
      const loginNis = document.getElementById('login-nis');
      const loginPass = document.getElementById('login-password');
      const teachPass = document.getElementById('teacher-password');
      if (loginNis) loginNis.value = '';
      if (loginPass) loginPass.value = 'user123';
      if (teachPass) teachPass.value = '';
    }
  },
  ...
  updatePlayerInfo() {
    ...
    playerInfoEl.innerHTML = `
      ...
      <div class="player-actions" style="grid-area: actions;">
        ...
        <button id="btn-header-logout" class="btn-action btn-action-danger" type="button" title="Keluar dari Akun">🚪 Keluar</button>
      </div>
  ...
  async updateRankingBanner() {
    ...
    // Default state: hide if player data is not set
    const data = typeof ProgressManager !== 'undefined' ? ProgressManager.getPlayerData() : null;
        ...
        ${slides.map((slide, i) => `
          <div class="banner-slide ${i === 0 ? 'active' : ''}" style="--slide-bg: url('${slide.image}');" data-index="${i}">
            <div class="banner-slide-content">
              ...
              <p class="banner-slide-desc">${slide.desc}</p>
            </div>
            <div class="banner-slide-action">
              <button type="button" class="banner-slide-btn">${slide.btnText}</button>
            </div>
          </div>
        `).join('')}
      </div>
    ...
    // Bind action to all slide buttons
    const buttons = bannerEl.querySelectorAll('.banner-slide-btn');
    buttons.forEach((btn, idx) => {
      btn.addEventListener('click', (e) => {
        e.stopPropagation();
        slides[idx].btnAction();
      });
    });

    // Set up slider logic variables
    let activeIdx = 0;
    const slidesElements = bannerEl.querySelectorAll('.banner-slide');
    const dotsElements = bannerEl.querySelectorAll('.banner-slider-dot');

    const changeSlide = (idx) => {
      activeIdx = idx;
      slidesElements.forEach((slide, sIdx) => {
        if (sIdx === activeIdx) {
    ...
    const dotsArea = bannerEl.querySelector('.banner-slider-dots');
    if (dotsArea) {
  ...
  openLevel(levelId) {
    ...
    const pemantikEl = document.getElementById('level-hub-pemantik');
    if (pemantikEl) {
      if (level.pertanyaanPemantik) {
        pemantikEl.innerHTML = `❓ <strong>Pertanyaan Pemantik:</strong> <em>"${level.pertanyaanPemantik}"</em>`;
        pemantikEl.style.display = 'block';
      } else {
        pemantikEl.style.display = 'none';
      }
    }
  ...
  _getLibraryIconSvg(levelId) {
    switch(levelId) {
      ...
      case 4:
        return `<svg width="70" height="70" viewBox="0 0 80 80" class="svg-lib-icon lib-components">
  ...
  /**
   * Menangani logout cepat mekanik/siswa dari menu utama.
   */
  handleQuickLogout() {
    this._showConfirmDialog(
  ...
  /**
   * Menangani logout dari panel guru.
   */
  handleTeacherLogout() {
    this.teacherSecret = null;
    sessionStorage.removeItem('automaster_teacher_secret');
    this.showScreen('landing');
    this.showToast('🚪 Berhasil keluar dari Panel Guru.', 'info');
  },
  ...
  updateCharts(students) {
    ...
    if (completionCtx) {
      ...
      this.charts.completion = new Chart(completionCtx, {
        type: 'bar',
        data: {
          labels: ['Lvl 1: Rem', 'Lvl 2: Mesin', 'Lvl 3: Kelistrikan', 'Lvl 4: Komponen', 'Lvl 5: BBM & Pendingin', 'Lvl 6: Ujian Akhir'],
          datasets: [{
  ...
  async loadStudentRankings() {
    ...
    if (this.charts.rankings) {
      ...
      this.charts.rankings = null;
    }

    const canvas = document.getElementById('chart-student-rankings');
    if (!canvas) return;

    const ctx = canvas.getContext('2d');
    let url = '/api?action=leaderboard&limit=15';
    if (window.location.protocol === 'file:') {
      url = 'http://localhost:8000/api?action=leaderboard&limit=15';
    }
    ...
    try {
      ...
      if (entries.length === 0) {
        ...
        ctx.fillText('Belum ada data prestasi siswa terdaftar.', canvas.width / 2, canvas.height / 2);
        return;
      }

      const chartLabels = entries.map((entry) => `#${entry.rank} - ${entry.nama}`);
      const chartValues = entries.map((entry, idx) => entries.length - idx);
      ...
      console.error('[App] Failed to load student rankings:', err);
      ctx.clearRect(0, 0, canvas.width, canvas.height);
      ctx.fillStyle = '#ef4444';
      ctx.font = '14px Inter';
      ctx.textAlign = 'center';
      ctx.fillText('⚠️ Hubungkan dengan server lokal untuk memuat peringkat.', canvas.width / 2, canvas.height / 2);
    }
  }
};
