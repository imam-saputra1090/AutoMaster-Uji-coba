...
<svg class="sim-bg-svg" viewBox="0 0 100 100" preserveAspectRatio="none" xmlns="http://www.w3.org/2000/svg">
  ...
  handleDragStart(e, componentId) {
    this.draggedElement = e.currentTarget || this._getClosestElement(e.target, '.component-card');
  ...
  handleDragEnter(e) {
    e.preventDefault();
    const zone = e.currentTarget || this._getClosestElement(e.target, '.drop-zone');
  ...
  /**
   * Helper tangguh untuk mencari elemen terdekat yang cocok dengan selector.
   * Mengatasi kasus di mana target adalah node teks (TEXT_NODE) yang tidak memiliki .closest().
   */
  _getClosestElement(target, selector) {
    if (!target) return null;
    let el = target;
    while (el && el.nodeType !== 1) { // 1 = Node.ELEMENT_NODE
      el = el.parentNode;
    }
