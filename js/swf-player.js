...
const SwfPlayer = {
  ...
  async play(swfPath, title) {
    ...
    // Allow trying to load the SWF directly. If CORS block happens, it will be caught in the try-catch block.
    ...
    try {
      ...
      if (typeof SWF_DATA !== 'undefined' && SWF_DATA[filename]) {
        console.log('[SwfPlayer] Memuat SWF dari memori (base64): ' + filename);
        var base64 = SWF_DATA[filename];
        ...
        await player.load({ data: bytes.buffer });
      } else {
        console.log('[SwfPlayer] Memuat SWF via URL fallback: ' + fullPath);
      ...
      if (window.location.protocol === 'file:') {
        this._showError(
          ...
          'Browser Chromium (Chrome/Edge) memblokir pemutaran file lokal (.swf) jika dibuka langsung (file://).\n\n' +
          'Solusi Praktis & Instan (Rekomendasi):\n' +
          '1. Klik ganda file "JALANKAN_GAME.bat" di dalam folder game Anda. File ini otomatis membuka game dengan izin file lokal yang tepat!\n\n' +
          'Solusi Lain:\n' +
          '2. Jalankan game menggunakan browser Firefox (mendukung pemutaran offline tanpa CORS block).\n' +
          '3. Jalankan server lokal (misalnya perintah "python -m http.server" di terminal pada folder game Anda).'
        );
      } else {
        this._showError(
...
const SWF_ANIMATIONS = {
  ...
  // ===== Level 4: Komponen Otomotif (12 files) =====
  4: [
    { file: 'crankshaft&piston.swf', title: 'Crankshaft & Piston', slide: 0, desc: 'Animasi crankshaft dan piston (Komponen Utama)' },
    { file: 'BLOCK CYL.swf', title: 'Blok Silinder', slide: 0, desc: 'Detail blok silinder mesin (Komponen Utama)' },
    { file: 'cylinder head.swf', title: 'Kepala Silinder', slide: 0, desc: 'Komponen kepala silinder (Komponen Utama)' },
    { file: 'CYL HEAD.swf', title: 'Detail Cylinder Head', slide: 0, desc: 'Detail cylinder head mesin (Komponen Utama)' },
    { file: 'camshaft-cam.swf', title: 'Camshaft & Nok', slide: 1, desc: 'Animasi camshaft dan nok (Komponen Tambahan)' },
    { file: 'camshaft-pushrod.swf', title: 'Pushrod Camshaft', slide: 1, desc: 'Sistem pushrod camshaft (Komponen Tambahan)' },
    { file: 'camshaft-sohc.swf', title: 'SOHC Camshaft', slide: 1, desc: 'Single Overhead Camshaft (Komponen Tambahan)' },
    { file: 'karter.swf', title: 'Karter / Oil Pan', slide: 1, desc: 'Komponen karter mesin (Komponen Tambahan)' },
    { file: 'ALTERNAT.swf', title: 'Alternator', slide: 2, desc: 'Animasi kerja alternator (Komponen Perlengkapan)' },
    { file: 'motor_starter.swf', title: 'Motor Starter', slide: 2, desc: 'Animasi kerja motor starter (Komponen Perlengkapan)' },
    { file: 'air filter.swf', title: 'Filter Udara', slide: 2, desc: 'Animasi filter udara mesin (Komponen Perlengkapan)' },
    { file: 'OTO cooling-system-pump.swf', title: 'Pompa Air', slide: 2, desc: 'Animasi pompa air pendingin (Komponen Perlengkapan)' }
  ],
